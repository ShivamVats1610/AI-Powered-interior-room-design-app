    ...
    <div className='mt-5'>
      ...
      <div className="grid grid-cols-2 mt-3 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {designs.map((design, index) => (
          <div
            key={index} onClick={()=>{setSelectedOption(design.name);selectedDesignType(design.name)}}
          >
            <Image
              src={design.image}
              width={100}
              height={100}
              alt={design.name}
              className={`object-contain h-[70px] rounded-md hover:scale-105 transition-all cursor-pointer ${design.name==selectedOption&&'border-2 border-primary rounded-md p-1'} `}
            />
            <h2>{design.name}</h2>
          </div>
        ))}
